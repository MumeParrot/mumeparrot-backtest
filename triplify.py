...
def main(input: str, output: str, rsi: str):
    ...
    input_prices = [(i[0], (float(i[1]), float(i[1]))) for i in input_chart[1:]]
    # input_prices = [(datetime.strptime(i[0], "%m/%d/%y").strftime("%Y-%m-%d"), (float(i[1]), float(i[2]))) for i in input_chart[1:]]
    ...
    with open(f"charts/{output}-SIM.csv", "w") as fd:
        merged_prices_str = [
            f"{i[0]},{i[1][0]},{i[1][1]}\n" for i in output_prices # Just use output_prices
        ]
